...
type Video struct {
	...
	Tags        StringArray  `gorm:"type:text;comment:标签" json:"tags"`
	Remarks     string       `gorm:"type:varchar(100);comment:备注" json:"remarks"`
	PlayLinks   StringArray  `gorm:"type:text;comment:播放链接 JSONB" json:"play_links"`
	Status      string       `gorm:"type:varchar(20);default:active;comment:状态" json:"status"`
	SourceID    uint         `gorm:"index;comment:来源采集源 ID" json:"source_id"`
...
type VideoSource struct {
	...
	SourceID  uint      `gorm:"index;not null;comment:采集源 ID" json:"source_id"`
...
type Episode struct {
	...
	SourceID uint   `gorm:"comment:来源采集源 ID" json:"source_id"`
}
...
type EpisodeSource struct {
	ID         uint   `gorm:"primaryKey" json:"id"`
	EpisodeID  uint   `gorm:"index;not null;comment:剧集 ID" json:"episode_id"`
	SourceID   uint   `gorm:"index;not null;comment:采集源 ID" json:"source_id"`
...
type UserWatchHistory struct {
	ID        uint      `gorm:"primaryKey" json:"id"`
	UserID    uint      `gorm:"uniqueIndex:idx_user_video;not null;comment:用户 ID" json:"user_id"`
	VideoID   uint      `gorm:"uniqueIndex:idx_user_video;not null;comment:视频 ID" json:"video_id"`
